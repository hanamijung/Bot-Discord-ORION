...
### ปรับปรุง
- แต่งหน้าตา `/staffpanel` embed ใหม่ทั้งหมด — ไอคอนจับคู่กับชื่อหมวดอัตโนมัติ (Owner=👑, Boss=💼, SubBoss=🎩, Admin=🛡️ ฯลฯ), สีไล่ระดับทอง→ม่วง→ฟ้าตามลำดับ order, thumbnail ไอคอน server, จัดรายชื่อเป็น blockquote พร้อมเลขลำดับและ mention คลิกได้
