...
- **บัค:** เคยลองทำให้กดจาก panel สาธารณะซ้ำหลายรอบไม่สร้าง ephemeral ใหม่ซ้อนกัน โดยจำ session ไว้แก้ไขข้อความเดิม แต่พบว่าถ้าผู้ใช้กด "Dismiss message" ปิดข้อความไปแล้ว บอทไม่มีทางรู้ได้เลยว่าถูกปิดไปแล้ว ทำให้ไปแก้ไขข้อความที่ผู้ใช้มองไม่เห็นแล้วสำเร็จเงียบๆ กลายเป็นดูเหมือนปุ่มไม่ตอบสนองเลย — ยกเลิกวิธีนี้แล้ว กลับไปให้กดจาก panel สาธารณะสร้าง ephemeral ใหม่เสมอ (ปลอดภัยกว่า)
...
- แก้ปัญหาข้อความ ephemeral ซ้อนกันเป็นกองเวลากดหลายหมวดต่อกัน — ข้อความ ephemeral มีปุ่มติดไปด้วย กดเปลี่ยนหมวดต่อ**จากในข้อความ ephemeral ของตัวเอง**จะแก้ไขข้อความเดิมแทนการสร้างใหม่ซ้อนกัน (เช็คจาก `interaction.message.flags` ว่าปุ่มถูกกดจาก panel สาธารณะหรือจากข้อความ ephemeral ของตัวเอง) — ส่วนการกดจาก **panel สาธารณะโดยตรงซ้ำหลายรอบ** ยังคงสร้าง ephemeral ใหม่ทุกครั้งโดยตั้งใจ (ดูหัวข้อ "แก้ไข" ด้านบนว่าทำไมถึงไม่พยายามแก้ข้อความเดิมในกรณีนี้)
- เพิ่ม `/categorylist` [แอดมิน] — เครื่องมือ diagnostic ดู raw ของชื่อหมวดทั้งหมด ช่วยตรวจจับอักขระแฝง/เว้นวรรคเกินที่อาจทำให้ปุ่มแสดงผลผิดปกติ
