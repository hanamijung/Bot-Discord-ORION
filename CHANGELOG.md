...
- แก้ปัญหาข้อความ ephemeral ซ้อนกันเป็นกองเวลากดหลายหมวดต่อกัน — ตอนนี้ข้อความ ephemeral มีปุ่มติดไปด้วย กดเปลี่ยนหมวดต่อจากในข้อความ ephemeral ของตัวเองจะ**แก้ไขข้อความเดิม**แทนการสร้างใหม่ซ้อนกัน (เช็คจาก `interaction.message.flags` ว่าปุ่มถูกกดจาก panel สาธารณะหรือจากข้อความ ephemeral ของตัวเอง)
- แก้ปัญหาเดียวกันสำหรับกรณีกดจาก **panel สาธารณะซ้ำหลายครั้ง** (ไม่ใช่แค่ในข้อความ ephemeral) — เก็บ session ของแต่ละคนไว้ใน memory (`staffPanelSessions`) พอกดหมวดใหม่จาก panel สาธารณะ จะ `editReply()` ของ ephemeral เดิมแทนการสร้างใหม่ ไม่ว่าจะกดกี่หมวดกี่ครั้งก็มีข้อความ ephemeral แค่อันเดียวต่อคน session หมดอายุอัตโนมัติใน 14 นาทีกันข้อมูลค้างในหน่วยความจำ
